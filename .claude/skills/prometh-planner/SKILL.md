...
if [ -n "$METADATA_TEMPLATE" ]; then
  ...
  FILE_CONTENT=$(cat "$PLAN_PATH")
  
  # Prepend YAML frontmatter and restore the file
  {
    echo "$METADATA_TEMPLATE"
    echo "$FILE_CONTENT"
  } > "$PLAN_PATH"
fi
```
...
### Step 5: Auto-run `pixel-planner timeline`
...
1. Write the combined frontmatter + document body to the output file.
...
```
No metadata template found in ${TRACKING_FILE}. Skipping metadata injection.
...
|-------|---------|--------|
| No CLAUDE.md | `Error: Claude configuration not found` | Exit, suggest `/prometh-init` |
