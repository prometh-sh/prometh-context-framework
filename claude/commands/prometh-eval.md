...
1. `CLAUDE.md` or `CLAUDE.local.md` exists in project root.
...
- If the `Verification` column contains a shell-executable command (e.g. `mise run test:unit`, `npm test`, `pytest path/to/test.py`), execute it via the `Bash` tool.
