...
Computational pre-commit sensors are dual-written to both the harness manifest and the `### Sensors` block in `AGENTS.md` / `CLAUDE.md` so the agent runs them every session.
